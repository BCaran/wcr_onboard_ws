...
#include "dynamixel_workbench_controllers/wcr_dynamixel_controller.h"
...
DynamixelController::DynamixelController()
  :node_handle_(""),
   ...
   is_moving_(false),
   is_wcr_robot_(false),
   wcr_robot_length_(0.0f),
   wcr_robot_width_(0.0f),
   wcr_robot_wheel_radius_(0.0f),
   D_(0.0f),
   Rx_(0.0f),
   Ry_(0.0f)
{
  ...
  is_wcr_robot_ = priv_node_handle_.param<bool>("use_wcr_robot", false);
  ...
  if (is_cmd_vel_topic_ == true){
    if (is_wcr_robot_ == true){
      wcr_robot_length_ = priv_node_handle_.param<double>("wcr_robot_config/robot_legth", 0.0);
      wcr_robot_width_ = priv_node_handle_.param<double>("wcr_robot_config/robot_width", 0.0);
      wcr_robot_wheel_radius_ = priv_node_handle_.param<double>("wcr_robot_config/wheel_radius", 0.0);
      D_ = sqrt((wcr_robot_length_ * wcr_robot_length_) + (wcr_robot_width_ * wcr_robot_width_));
      Rx_ = wcr_robot_length_ / D_;
      Ry_ = wcr_robot_width_ / D_;
    }
    else{
...
void DynamixelController::initSubscriber()
{
  ...
  if (is_cmd_vel_topic_){
    if (is_wcr_robot_)
      cmd_vel_sub_ = priv_node_handle_.subscribe("cmd_vel", 10, &DynamixelController::wcrCommandVelocityCallback, this);
    else
...
void DynamixelController::optimazeSpeedAngle(double returnSpeedAngle[], double speed, double angle){
...
//Callback funkcija za upravljanje wcr s 4 kotača
void DynamixelController::wcrCommandVelocityCallback(const geometry_msgs::Twist::ConstPtr &msg){
  
  bool result = false;
  const char* log = NULL;

  uint8_t velocity_id_array[4] = {FL_WHEEL, FR_WHEEL, BL_WHEEL, BR_WHEEL}; //ID's for each motor that produces speed of robot
  uint8_t position_id_array[4] = {FL_POSITION, FR_POSITION, BL_POSITION, BR_POSITION}; //ID's for each motor that rotates speed motor
  ...
  double position_array_deg[4] = {0.0, 0.0, 0.0, 0.0}; //angle that each motor need's to make
  ...
  double velocity_constant_value = 1 / (0.229 * 0.10472 * wcr_robot_wheel_radius_);
  ...
  position_array_deg[0] = atan2(outputSpeedsVxVy[1], outputSpeedsVxVy[0]) * 180 / PI;
  position_array_deg[1] = atan2(outputSpeedsVxVy[3], outputSpeedsVxVy[2]) * 180 / PI;
  position_array_deg[2] = atan2(outputSpeedsVxVy[5], outputSpeedsVxVy[4]) * 180 / PI;
  position_array_deg[3] = atan2(outputSpeedsVxVy[7], outputSpeedsVxVy[6]) * 180 / PI;

  double returnValueTemp[2];
  for(int i = 0;i < 4;i++){
    optimazeSpeedAngle(returnValueTemp, velocity_array_m_s[i], position_array_deg[i]);
    velocity_array_m_s[i] = returnValueTemp[0];
    position_array_deg[i] = returnValueTemp[1];
  }

  for(int i = 0;i < 4;i++)
    dynamixel_position[i] = FL_INIT_POSITION + position_array_deg[i] * PULS_PER_DEG;

  for(int i = 0;i < 4;i++)
    dynamixel_velocity[i] = velocity_array_m_s[i] * velocity_constant_value;

  result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION, position_id_array, 4, dynamixel_position  , 1, &log);
  if (result == false){
    ROS_ERROR("%s", log);
  }

  result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_VELOCITY, velocity_id_array, 4, dynamixel_velocity  , 1, &log);
  if (result == false){
    ROS_ERROR("%s", log);
  }
}
...
void DynamixelController::initStartPosition(){
  bool result = false;
  const char* log = NULL;
  uint8_t id_array[4] = {FL_POSITION, FR_POSITION, BL_POSITION, BR_POSITION};
  int32_t dynamixel_position[4] = {FL_INIT_POSITION, FR_INIT_POSITION, BL_INIT_POSITION, BR_INIT_POSITION};
  
  result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION, id_array, 4, dynamixel_position, 1, &log);
  if (result == false){
    ROS_ERROR("%s", log);
  }
}
...
int main(int argc, char **argv)
{
  ...
  dynamixel_controller.initStartPosition();
